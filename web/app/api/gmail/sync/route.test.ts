...
describe("POST /api/gmail/sync", () => {
  ...
  it("requires a verified session", async () => {
    ...
    expect((await POST(new Request("http://localhost/api/gmail/sync", { method: "POST" }))).status).toBe(401);
  });
  ...
  it("does not start sync when Gmail is disconnected", async () => {
    ...
    const response = await POST(new Request("http://localhost/api/gmail/sync", { method: "POST" }));
