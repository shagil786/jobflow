...
describe("POST /api/gmail/sync", () => {
  ...
  it("preserves actionable guidance when the scoped Gmail label is missing", async () => {
    getServerSessionMetadata.mockResolvedValue({ userId: "user-1", tenantId: "tenant-1" });
    vi.stubEnv("JOBFLOW_INTERNAL_SERVICE_KEY", "internal-test-key");
    vi.spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(new Response(JSON.stringify({ connected: true, connectionId: "connection-1" }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ error: { code: "GMAIL_TRACK_LABEL_NOT_FOUND" } }), { status: 422 }));
    const response = await POST();
    expect(response.status).toBe(422);
    expect(await response.json()).toMatchObject({ error: { code: "GMAIL_TRACK_LABEL_NOT_FOUND", message: "Create the Gmail label JobFlow/Track, then sync again" } });
  });
});
