...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(`${readInternalServiceUrl("JOB_SERVICE_URL", "http://localhost:8091")}/api/v1/applications/from-reviewed-message`, { method: "POST", headers: { "content-type": "application/json", authorization }, body: await request.text(), cache: "no-store" });
