...
async function proxy(method: "GET") {
  ...
  try {
    ...
    const response = await fetch(`${readInternalServiceUrl("INGESTION_SERVICE_URL", "http://localhost:8082")}/internal/v1/classification-suggestions/review-queue?${query}`, { method, headers: { "X-Internal-Service-Key": key }, cache: "no-store", signal: AbortSignal.timeout(5000) });
    if (!response.ok) return NextResponse.json({ error: { code: "INGESTION_UNAVAILABLE", message: "Review queue is unavailable" }, meta: {} }, { status: 503 });
    return new NextResponse(await response.text(), { status: 200, headers: { "content-type": "application/json" } });
  } catch {
    return NextResponse.json({ error: { code: "INGESTION_UNAVAILABLE", message: "Review queue is unavailable" }, meta: {} }, { status: 503 });
  }
}
