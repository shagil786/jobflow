      ...
      <main className="main">
        ...
        <div className="content-grid">
          <section className="section-card action-card" aria-labelledby="actions-heading">
            <div className="section-head"><div><span className="section-overline">Your working queue</span><h2 id="actions-heading" className="section-title">Today&apos;s next actions</h2><p className="section-caption">A focused queue built from your confirmed timeline.</p></div><span className="count-pill">{filter === "Rejected" ? rejectedActions.length : actions.length} {filter === "Rejected" ? "rejected" : "open"}</span></div>
            <div className="filter-row" role="toolbar" aria-label="Filter next actions">{["All", "Follow up", "Review reply", "Prepare", "Rejected"].map((item) => <button key={item} className={`filter ${filter === item ? "active" : ""}`} type="button" onClick={() => setFilter(item)}>{item}</button>)}</div>
            <div className="action-list">{filter === "Rejected" ? visibleRejected.length ? visibleRejected.map((action) => <ActionRow key={action.id} action={action} onComplete={complete} onOpen={() => action.sourceMessageId ? setActiveThread({ messageId: action.sourceMessageId, title: action.title }) : notify("This older record has no stored Gmail message reference, so its conversation cannot be opened.")} />) : <div className="empty-state queue-empty"><div className="empty-icon" aria-hidden="true">✓</div><strong>No rejected roles.</strong><span>Verified rejection messages will appear here.</span></div> : visibleActions.length ? visibleActions.map((action) => <ActionRow key={action.id} action={action} onComplete={complete} onOpen={() => action.sourceMessageId ? setActiveThread({ messageId: action.sourceMessageId, title: action.title }) : notify("This older record has no stored Gmail message reference, so its conversation cannot be opened.")} />) : <div className="empty-state queue-empty"><div className="empty-icon" aria-hidden="true">◌</div><strong>Your queue is clear.</strong><span>Capture a role or scan Gmail to create your first real next action.</span><button className="text-button" type="button" onClick={() => setCaptureOpen(true)} disabled={loadState !== "ready"}>Capture your first role <span aria-hidden="true">→</span></button></div>}</div>
          </section>

          <div>
            {reviewLoadState === "unavailable" ? <section className="section-card review-card" aria-labelledby="review-heading"><div className="section-head"><div><span className="section-overline">Evidence inbox</span><h2 id="review-heading" className="section-title">Needs your review</h2></div></div><div className="empty-state">Review queue is unavailable. No local suggestions are shown.</div></section> : <GmailReviewQueue items={reviews} savingId={reviewSaving} onDecision={saveReview} hasMore={reviewHasMore} loadingMore={reviewLoadingMore} onLoadMore={loadMoreReviews} />}
